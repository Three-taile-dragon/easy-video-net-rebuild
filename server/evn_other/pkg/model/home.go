...
func (r *GetHomeInfoResponse) Response(rotographList *rotograph.List, videoList *video.VideosContributionList, localhost string, tencentOssHost string) {
	//处理轮播图
	rl := make(rotographInfoList, 0)
	for _, rk := range *rotographList {
		cover, _ := conversion.FormattingJsonSrc(rk.Cover, localhost, tencentOssHost)
		info := rotographInfo{
			Title: rk.Title,
			Cover: cover,
			Color: rk.Color,
