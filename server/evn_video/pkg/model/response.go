...
func GetVideoContributionCommentsResponse(vc *video.VideosContribution, localhost string, tencentOssHost string) GetArticleContributionCommentsResponseStruct {
	//评论
	...
	for _, v := range vc.Comments {
		...
		commentUser, _ := videoRepo.GetUserByID(ctx, uint32(v.CommentUserID))
...
func GetVideoContributionByIDResponse(vc *video.VideosContribution, recommendVideoList *video.VideosContributionList, isAttention bool, isLike bool, isCollect bool, localhost string, tencentOssHost string) Response {
	//处理视频主要信息
	creatorAvatar, _ := conversion.FormattingJsonSrc(vc.UserInfo.Photo, localhost, tencentOssHost)
	cover, _ := conversion.FormattingJsonSrc(vc.Cover, localhost, tencentOssHost)
	videoSrc, _ := conversion.FormattingJsonSrc(vc.Video, localhost, tencentOssHost)
	video720pSrc, _ := conversion.FormattingJsonSrc(vc.Video720p, localhost, tencentOssHost)
	video480pSrc, _ := conversion.FormattingJsonSrc(vc.Video480p, localhost, tencentOssHost)
	video360pSrc, _ := conversion.FormattingJsonSrc(vc.Video360p, localhost, tencentOssHost)
	//评论
	comments := commentsInfoList{}
	for _, v := range vc.Comments {
		photo, _ := conversion.FormattingJsonSrc(v.UserInfo.Photo, localhost, tencentOssHost)
...
func GetVideoManagementListResponse(vc *video.VideosContributionList, localhost string, tencentOssHost string) (interface{}, error) {
	...
	for _, v := range *vc {
		...
		videoSrc, _ := conversion.FormattingJsonSrc(v.Video, localhost, tencentOssHost)
