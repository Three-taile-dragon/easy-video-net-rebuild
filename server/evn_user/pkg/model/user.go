...
func UserInfoResponse(us *user.User, token string, localhost string, tencentOssHost string) UserInfoResponseStruct {
	//判断用户是否为微信用户进行图片处理
	photo, _ := conversion.FormattingJsonSrc(us.Photo, localhost, tencentOssHost)
...
func UserSetInfoResponse(us *user.User) UserSetInfoResponseStruct {
...
func GetSpaceIndividualResponse(us *user.User, isAttention bool, attentionNum *int64, vermicelliNum *int64, localhost string, tencentOssHost string) (data any, err error) {
	photo, _ := conversion.FormattingJsonSrc(us.Photo, localhost, tencentOssHost)
...
func GetReleaseInformationResponse(videoList *video.VideosContributionList, articleList *article.ArticlesContributionList, localhost string, tencentOssHost string) (data interface{}, err error) {
	//处理视频
...
func GetAttentionListResponse(al *attention.AttentionsList, arr []uint, localhost string, tencentOssHost string) (data interface{}, err error) {
	list := make(GetAttentionListInfoList, 0)
	for _, v := range *al {
		photo, _ := conversion.FormattingJsonSrc(v.AttentionUserInfo.Photo, localhost, tencentOssHost)
		isAttention := false
		for _, ak := range arr {
			if ak == v.AttentionID {
				isAttention = true
...
func GetVermicelliListResponse(al *attention.AttentionsList, arr []uint, localhost string, tencentOssHost string) (data interface{}, err error) {
...
func GetFavoritesListResponse(al *favorites.FavoriteList, localhost string, tencentOssHost string) (data interface{}, err error) {
...
func GetFavoritesListByFavoriteVideoResponse(al *favorites.FavoriteList, ids []uint, localhost string, tencentOssHost string) (data interface{}, err error) {
...
func GetFavoriteVideoListResponse(cl *collect.CollectsList, localhost string, tencentOssHost string) (data interface{}, err error) {
	//处理视频
...
func GetRecordListResponse(rl *record.RecordsList, localhost string, tencentOssHost string) (data interface{}, err error) {
	...
	for _, v := range *rl {
		...
		if v.Type == "video" {
			...
			photo, _ = conversion.FormattingJsonSrc(v.VideoInfo.UserInfo.Photo, localhost, tencentOssHost)
			...
			photo, _ = conversion.FormattingJsonSrc(v.ArticleInfo.UserInfo.Photo, localhost, tencentOssHost)
			title = v.ArticleInfo.Title
			username = v.ArticleInfo.UserInfo.Username
			tp = "专栏"
		} else {
			cover, _ = conversion.FormattingJsonSrc(v.Userinfo.LiveInfo.Img, localhost, tencentOssHost)
			photo, _ = conversion.FormattingJsonSrc(v.Userinfo.Photo, localhost, tencentOssHost)
...
func GetNoticeListResponse(nl *notice.NoticesList, localhost string, tencentOssHost string) (data interface{}, err error) {
...
func GetChatListResponse(chatList *chatList.ChatList, msgList map[uint]*chatMsg.MsgList, localhost string, tencentOssHost string) (data interface{}, err error) {
	list := make(GetChatListResponseStruct, 0)
	for _, v := range *chatList {
		photo, _ := conversion.FormattingJsonSrc(v.ToUserInfo.Photo, localhost, tencentOssHost)
		messageList := make([]ChatMessageInfo, 0)
		for _, vv := range *msgList[v.Tid] {
			uPhoto, _ := conversion.FormattingJsonSrc(vv.UInfo.Photo, localhost, tencentOssHost)
...
func GetChatHistoryMsgResponse(list *chatMsg.MsgList, localhost string, tencentOssHost string) (data interface{}, err error) {
	messageList := make([]ChatMessageInfo, 0)
	for _, v := range *list {
		photo, _ := conversion.FormattingJsonSrc(v.UInfo.Photo, localhost, tencentOssHost)
		messageList = append(messageList, ChatMessageInfo{
			ID:        v.ID,
			Uid:       v.Uid,
